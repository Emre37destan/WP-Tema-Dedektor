					...
					function prepare_json(res) {
						...
						if (typeof res.data !== 'undefined' && typeof res.data.theme_count !== 'undefined') {
							...
							if (res.data.theme_count == 0 && res.data.plugin_count == 0) {
								tpl = '<div id="js_no_wp" class="error_alert" style="display:block">Bu site WordPress kullanmiyor gibi gorunuyor.</div>';
							}
...
<div class="panel-heading" style="background-color:#2E2D2E;padding: 10px 25px;
    border-bottom: 1px solid transparent;"><h3 class="panel-title" style="font-size:13px;color:white;text-shadow:rgba(0, 0, 0, 0.219608) 0 1px 0;margin:0px;">Algilanan WordPress Temasi ${data.theme_count > 1 ? 's' : ''} (${data.theme_count}x)<a href="https://themesinfo.com" target="_blank" style="text-align:right;font-size:12px;float:right;color:#c6c6c6;font-weight: normal">Gelistiren <b>KralDev</b>. Yeni Sistem</a></h3>
...
<div class="starter-template" style="border-radius: 6px;margin:0 10px;padding:0px 10px 0 10px;">
            ...
            <table class="table" style="font-size: 13px;color:#666;word-break: break-all;white-space: nowrap;">
                <tr>
                    <td style="width: 30%;border-top: none;padding-top: 1px;"><i class="fa fa-picture-o theme_i"
                                                                                 aria-hidden="true"></i> Tema Adi:
                    </td>
                    <td style="border-top: none;padding-top: 1px;">

                        ${theme.name}
                        ${theme.theme_name != '' ? `<a href="${theme.theme_url}" title="${theme.name}" target="blank">Daha fazla goster</a>` : ''}
                    </td>
                </tr>

                <tr>
                    <td style="border-top: none;"><i class="fa fa-user-o theme_i" aria-hidden="true"></i> Sahip:</td>
                    <td style="border-top: none;">
                        ${theme.author}
                        ${theme.author != '' ? `<a href="${theme.author_url}" title="${theme.author}" target="blank">Daha fazla goster</a>` : ''}
                    </td>
                </tr>

                ${theme.usage_count != 0 ? `
                <tr>
                <td style="border-top: none;"><i class="fa fa-globe theme_i" aria-hidden="true"></i> Kullanilan tema:</td>
                <td style="border-top: none;">
                <b>${theme.usage_count}</b> website${theme.usage_count != 0 ? 's' : ''}  ${theme.theme_name != '' ? `<a href="${theme.theme_url}" title="${theme.name}" target="blank">Daha fazla goster</a>` : ''}
                </td>
                </tr>
                ` : ''}

                <tr>
                    <td style="border-top: none;"><i class="fa fa-folder-o theme_i" aria-hidden="true"></i> Tema&nbsp;Klasoru:
                    </td>
                    <td style="border-top: none;">
                        ${theme.folder}
                        ${theme.folder != '' ? `<a href="${theme.folder_url}" title="${theme.name}" target="blank">Daha fazla goster</a>` : ''}
                    </td>
                </tr>

                <tr>
                    <td style="border-top: none;"><i class="fa fa-clone theme_i" aria-hidden="true"></i> Versiyon:</td>
                    <td style="border-top: none;">${theme.version}</td>
                </tr>

                <tr>
                    <td style="border-top: none;"><i class="fa fa-desktop theme_i" aria-hidden="true"></i> Tema&nbsp;Ana Sayfa:
                    </td>
                    <td style="border-top: none;">
                ...
                <tr>
                    <td style="border-top: none;"><i class="fa fa-desktop theme_i" aria-hidden="true"></i> Sahip
                        Ana Sayfa:
                    </td>
                    <td style="border-top: none;">
    ...
    ${theme.description != '' ? `<p style="font-size:13px;margin: 5px;margin-bottom: 15px"><b>Aciklama:</b> ${theme.description}</p>` : ''}
       ...
       ${data.plugin_count > 0 ? `
		<div class="panel-heading" style="background-color:#2E2D2E;padding: 10px 25px;border-bottom: 1px solid transparent;margin-bottom: 10px">
					<h3 class="panel-title" style="font-size:13px;color:white;text-shadow:rgba(0, 0, 0, 0.219608) 0 1px 0;margin:0px;">
						Algilanan WordPress Eklentisi ${data.plugin_count > 1 ? 's' : ''} (${data.plugin_count}x)
					</h3>
				</div>
		` : ``}
...
<div class="col-sm-6 col-md-6" style="margin-top: 10px;padding:0 8px">
    ...
    <div style="border: 1px solid #eeeeee;">
		...
		`<img src="${plugin.img}" style="float:left;margin-right:15px;width:60px" alt="${plugin.title}" title="${plugin.title}">`}
            ...
            ${plugin.type == 'Free' ? '<span style="font-size:10px;color:green;float:right">Bedava</span>' : ''}
            ${plugin.type == 'Premium' ? '<span style="font-size:10px;color:red;float:right">Ucretli</span>' : ''}
		...
		`<h3 style="font-size:16px;margin-top: 0px;margin-bottom:2px;">${plugin.plugin_name}</h3>`}
                ...
                <span style="">${plugin.description != '' ? `${plugin.description}` : `Bilinmeyen eklenti...`}</span>
            </div>
...
`.trim()).join('')}
  ...
  <span class="server_ok"></span>
	<center style=";margin-top: 12px"><a href="https://themesinfo.com" target="_blank" style="text-align:center;text-shadow: 0 1px 0 #fff;color: #857f78;font-size:12px;font-weight: normal">Gelistiren <b>KralDev</b>. Yeni Sistem</a></center>
	`;
